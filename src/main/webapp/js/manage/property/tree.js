...
function loadDepTree(){
	$('#depTreeul').tree({
	    url:path+'/business/businessDepartment/treeData.do?type='+type+'&id='+proId,
	    method: 'post',
	    animate: true,
...
function append2(){
    ...
    var stu = $.post(path+'/business/businessPosition/save.do',{parentId: node.id,parentName:node.text,orgId:proId,orgName:proName,depId:depId,depName:depName,orgType:type},function(date){
        if(date.status == 'ERROR'){
            $.messager.alert('系统提示', '未成功请联系管理员!', 'warning');
        }else{
        	 t.tree('append', {
...
function append3(){
    var stu = $.post(path+'/business/businessDepartment/save.do',{orgId:proId,orgName:proName,orgType:type},function(date){
        if(date.status == 'ERROR'){
            $.messager.alert('系统提示', '未成功请联系管理员!', 'warning');
        }else{
