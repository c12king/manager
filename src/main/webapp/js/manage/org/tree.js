...
function append2(){
    ...
    var stu = $.post(path+'/business/businessPosition/save.do',{parentId: node.id,parentName:node.text,orgId:orgId,orgName:orgName,depId:depId,depName:depName,orgType:attr.type},function(date){
        if(date.status == 'ERROR'){
            $.messager.alert('系统提示', '未成功请联系管理员!', 'warning');
        }else{
        	 t.tree('append', {
...
function append3(){
    ...
    var stu = $.post(path+'/business/businessDepartment/save.do',{orgId:orgId,orgName:orgName,orgType:attr.type},function(date){
        if(date.status == 'ERROR'){
            $.messager.alert('系统提示', '未成功请联系管理员!', 'warning');
        }else{
