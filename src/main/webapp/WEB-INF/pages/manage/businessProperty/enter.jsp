...
$(function() {
	...
	grid = $('#grid').datagrid({
        title: 'BusinessProperty管理',
        iconCls: 'icon-save',
        url: '<%=path %>/manage/businessProperty/list.do',
        fit: true,
        fitColumns: true,
        queryParams:{
        	proName:function(){
        ...
        columns: [[
    		          { field: 'proName', title: '物业名称', width: 150, align:'center' },
    		          { field: 'proId', title: 'ID', width: 150, align:'center' },
    		          { field: 'proBrief', title: '物业简介', width: 150, align:'center' },
    		          { field: 'proService', title: '物业服务说明', width: 150, align:'center' },
    		          { field: 'proTel', title: '物业电话', width: 150, align:'center' },
    		          { field: 'proEmail', title: '物业邮件', width: 150, align:'center' },
    		          { field: 'proWeixin', title: '物业微信', width: 150, align:'center' },
    		          { field: 'proIcon', title: '物业图标', width: 150, align:'center' },
    		          { field: 'proLongitude', title: '物业经度', width: 150, align:'center' },
    		          { field: 'proLatitude', title: '物业纬度', width: 150, align:'center' },
...
function add() {
	win = $('#add-window').window({
        ...
        shadow: false,
        cache: false,
        href: '<%=path %>/manage/businessProperty/add.do'
    });
...
function edit() {
    ...
    if (num == 0) {
    	...
    	win = $('#edit-window').window({
            ...
            shadow: false,
            cache: false,
            href: '<%=path %>/manage/businessProperty/modify.do?proId='+rows[0].proId
            
        });
    	$('#edit-window').window('open');
        $('#modifyForm').form('clear');
        $('#modifyForm').form('load', '<%=path %>/manage/businessProperty/edit.do?id=' + rows[0].proId);
    }
}
...
function del() {
    ...
    if (arr.length>0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: '<%=path %>/manage/businessProperty/delete.do?id=' + arr2str(arr),
                    timeout: 1000,
                    cache: false,
...
function department_management(){
    ...
    if (num == 0) {
        ...
        return;
    	 ...
    	 var url='<%=path %>/manage/businessProperty/org.do?id='+rows[0].proId
         var title="部门管理"
         var obj ={
             title:title,
             url:url
...
<body class="easyui-layout" style="overflow-y: hidden;">
    ...
    <div id="search-window" title="查询窗口" style="width: 400px; height: 500px;">
        <div style="padding: 20px 20px 40px 80px;">
            <form id="searchForm" method="post" action="<%=path %>/manage/businessProperty/list.do">
            <table>
					<tr>
			          <td>物业名称：</td>
			          <td>
			          	<input name="proName" id="proName" type="text" style="width: 150px;" value=""/>
			          </td>
			        </tr>
            </table>
            </form>
