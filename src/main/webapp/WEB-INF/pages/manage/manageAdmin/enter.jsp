...
$(function() {
	...
	grid = $('#grid').datagrid({
        title: 'ManageAdmin管理',
        iconCls: 'icon-save',
        url: '<%=basePath %>/manage/manageAdmin/list.do',
        fit: true,
        fitColumns: true,
		resize: true,
        sortName: 'adminId',
        ...
        columns: [[
    		          { field: 'adminName', title: '管理员账号', width: 150, align:'center' },
    		          { field: 'adminPassword', title: '管理员密码', width: 150, align:'center' },
    		          { field: 'adminType', title: '角色类型', width: 150, align:'center' },
                ]],
        pagination: true,
        pageNumber: 1,
        pageSize: 10,
...
function add() {
	win = $('#add-window').window({
        ...
        shadow: false,
        cache: false,
        href: '<%=basePath %>/manage/manageAdmin/add.do'
    });
...
function edit() {
    ...
    if (num == 0) {
    	...
    	win = $('#edit-window').window({
            ...
            shadow: false,
            cache: false,
            href: '<%=basePath %>/manage/manageAdmin/modify.do?adminId='+rows[0].adminId
...
function del() {
    ...
    if (arr.length>0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: '<%=basePath %>/manage/manageAdmin/delete.do?id=' + arr2str(arr),
                    timeout: 1000,
                    cache: false,
...
<body class="easyui-layout" style="overflow-y: hidden;">
    ...
    <div id="search-window" title="查询窗口" style="width: 400px; height: 300px;">
        <div style="padding: 20px 20px 40px 80px;">
            <form id="searchForm" method="post" action="<%=basePath %>/manage/manageAdmin/list.do">
            <table>
					<tr>
			          <td>管理员账号：</td>
			          <td>
			          	<input name="adminName" id="adminName" type="text" style="width: 150px;" value=""/>
			          </td>
			        </tr>
					<tr>
			          <td>管理员密码：</td>
			          <td>
			          	<input name="adminPassword" id="adminPassword" type="text" style="width: 150px;" value=""/>
			          </td>
			        </tr>
					<tr>
			          <td>角色类型：</td>
			          <td>
			          	<input name="adminType" id="adminType" type="text" style="width: 150px;" value=""/>
			          	<select name="adminType" id="adminType" class="selectwidth">
