...
$(function() {
	...
	grid = $('#grid').datagrid({
        title: 'ManageModule管理',
        iconCls: 'icon-save',
        url: '<%=path %>/manage/manageModule/list.do',
        fit: true,
        fitColumns: true,
        queryParams:{
        	moduleName:function(){
        ...
        columns: [[
    		          ...
    		          { field: 'moduleDesc', title: '模块描述', width: 150, align:'center' },
    		          { field: 'moduleIcon', title: '模块图标', width: 150, align:'center' },
    		          { field: 'moduleUrl', title: '模块链接', width: 150, align:'center' },
...
function add() {
	win = $('#add-window').window({
        ...
        shadow: false,
        cache: false,
        href: '<%=path %>/manage/manageModule/add.do'
    });
...
function edit() {
    ...
    if (num == 0) {
    	...
    	win = $('#edit-window').window({
            ...
            shadow: false,
            cache: false,
            href: '<%=path %>/manage/manageModule/modify.do?moduleId='+rows[0].moduleId
...
function assignMenu() {
    ...
    if (num == 0) {
    	...
    	win = $('#assignMenu-window').window({
            closed: true,
            modal: true,
            shadow: false,
            cache: false,
            href: '<%=path %>/manage/businessMenu/assignMenu.do?id='+rows[0].moduleId
            
        });
    	$('#assignMenu-window').window('open');
    }
}
...
function del() {
    ...
    if (arr.length>0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: '<%=path %>/manage/manageModule/delete.do?id=' + arr2str(arr),
                    timeout: 1000,
                    cache: false,
...
<body class="easyui-layout" style="overflow-y: hidden;">
    ...
    <div id="assignMenu-window" title="新增窗口" style="width: 500px; height: 500px;"></div>
    ...
    <div id="search-window" title="查询窗口" style="width: 400px; height: 300px;">
        <div style="padding: 20px 20px 40px 80px;">
            <form id="searchForm" method="post" action="<%=path %>/manage/manageModule/list.do">
            <table>
					<tr>
			          <td>模块名称：</td>
			          <td>
			          	<input name="moduleName" id="moduleName" type="text" style="width: 150px;" value=""/>
			          </td>
			        </tr>
            </table>
            </form>
